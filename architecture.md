...
- **ouverte** : responsable unique vs co-responsabilité art. 26 (DPO opérateur).
...
| Concern | Implémentation cible | Risque / NFR mitigé | — |---|---|---| — | Audit immuable | `axia_audit_event` + `pg_partman` (mensuel) + WORM S3 | — |
| Sécurité secrets | OCA `keychain` + clé maître hors-base | — |
| Observabilité J1 | Prometheus + alerting + dashboard Odoo + Loki | (mode pionnier), | — | RBAC 7 rôles + délégation | Groupes Odoo + record rules + gating actions techniques | / | — | Idempotence | Outbox Odoo `(payload_hash, response)` | — |
...
| Calendrier multi-pays | `python-holidays` + surcharge admin sans code | CAP-9 | — | PPP Odoo-owned | Algo génération + rotation gated + chiffrement |, | — | Anti-replay webhook | UNIQUE INDEX `event_id` fenêtre 5 min + HMAC constant-time | — |
| Reprise crash queue_job | Runbook + alerting `started_for > 30m` | — |

### Open Questions critiques (ADR Phase 0)

| OQ | Sujet | Impact architecture | — |---|---|---| — | **** | Payloads réels POST customer / internet-service | Bloque la couche Mapper — POC tenant Splynx | — | **a** | Reprise PPP legacy (bascule v1→v2) | Import passif, freeze baseline, audit `ppp.imported.legacy` | — | **** | Statut juridique (resp. unique vs co-resp art. 26) | Multi-company vs multi-DB | — | **** | Algorithme/format PPP généré Odoo | Charset, entropie, longueur, anti-collision |
...
**Pourquoi ce choix** :
- Les décisions de stack sont figées par le.
...
| ADR | Sujet | OQ adressée | — |---|---|---| — | ADR-001 | Payloads Splynx & contrat Mapper | — |
...
| ADR | Sujet | — |---|---| — | ADR-005 | Schéma `axia_audit_event` & partitionnement | — | ADR-006 | Sécurité webhooks Splynx | — | ADR-007 | Authentification API Splynx | — | ADR-008 | Client HTTP Splynx (`httpx` custom) | — | ADR-009 | Fairness queue_job multi-tenant | — | ADR-010 | Observabilité J1 (mode pionnier) |

**Deferred Decisions (Sprint 0 ops ou post-MVP)** :
| OQ | Sujet | Différé jusqu'à | — |---|---|---| — | | Maturité OCA 18.0 | Sprint 0 (audit pré-pinning) | — | | Versions MySQL/MariaDB Splynx | Sprint 0 ops | — | | Maturité Sentry + queue_job | Sprint 0 | — | | Connecteur Splynx propriétaire Apps Store | Recherche PM | — | | Stabilité IP tenant Splynx (allow-list) | Contact Splynx | — | | Format export comptable | PM + opérateur | — | | Hébergement multi-pays | Projet infra séparé | — | | Marqueur ISP service (`is_isp_service` source) | PM + opérateur |
...
**Exemples canoniques** :
| Domaine | Events | — |---|---| — | customer | `customer.synced`, `customer.sync.failed`, `customer.created`, `customer.terminated` | — | ppp | `ppp.generated`, `ppp.rotated`, `ppp.revealed`, `ppp.imported.legacy` | — | suspension | `suspension.executed`, `suspension.postponed`, `suspension.cancelled` | — | reactivation | `reactivation.executed`, `reactivation.failed` | — | role | `role.granted`, `role.revoked`, `partner.delegated` | — | webhook | `webhook.received`, `webhook.duplicate`, `webhook.invalid_hmac` | — | mapper | `mapper.upgraded` |
...
| Channel | Usage | Concurrency | — |---|---|---| — | `root.sync.marque-a` | Sync Marque A | 1 (anti rate-limit) | — | `root.sync.marque-b` | Sync Marque B | 1 | — | `root.sync.marque-c` | Sync Marque C | 1 | — | `root.sync.marque-d` | Sync Marque D | 1 | — | `root.webhook` | Webhooks entrants (toutes marques) | 5 (faible latence) | — | `root.reconcile` | Cron horaire de réconciliation | 1 (basse prio) | — | `root.legacy_import` | Import PPP legacy Sprint 1 | 1 (one-shot) |
...
| Binding | Modèle | — |---|---| — | `axia.binding.splynx.customer` | Client | — | `axia.binding.splynx.internet_service` | Service Internet | — | `axia.binding.splynx.ppp` | Identifiants PPP |
...
| Fichier | Contenu | — |---|---| — | `security/ir.model.access.csv` | ACL standard Odoo | — | `security/security.xml` | Groupes + record rules | — | `data/<entity>.xml` | Data techniques (event_types, channels…) | — | `demo/<entity>.xml` | Données démo (`noupdate="1"`) |
...
| Type | Action | Conséquence | — |---|---|---| — | **Retryable** (timeout, 5xx, 429) | `raise RetryableJobError(msg, seconds=N)` | Retry exponentiel auto | — | **Permanent** (4xx hors 429, validation Odoo, contradiction métier) | `raise FailedJobError(msg)` | Job état `failed` + alert Prometheus | — | **Inattendu** (bug) | `raise` brut | Job état `failed` + remontée Sentry |
...
**Vérification** :
| Pattern | Méthode de vérif | — |---|---| — | `event_type` déclaré | Boot test : raise si event_type non déclaré au `_audit` call | — | `correlation_id` propagé | Test d'intégration E2E + grep logs | — | Schéma payload audit | Validation pydantic/jsonschema dans `_audit` | — | Channels nommage | Pre-commit hook qui parse `__manifest__.py` + `odoo.conf` | — | `@requires_role` sur actions | Pylint custom rule (à écrire `axia_rbac/pylint_plugin.py`) | — | Pas de `try/except Exception: pass` | `pylint-odoo` enforce | — | Pas de secret dans logs | CI : grep `Pass ppp` / `token` dans output | — | Manifest deps explicites | `pylint-odoo` rule `missing-manifest-dependency` |
...
| Capability | FRs principaux | Module(s) primaire(s) | Modules consommateurs | — |---|---|---|---| — | CAP-1 (création client) | /d/e | `axia_splynx_connector` | `axia_audit`, `axia_rbac` | — | CAP-2 (déduplication) | — | `axia_splynx_connector` | — | — | CAP-3 (sync incrémentale) | — | `axia_splynx_connector` | — | — | CAP-4 (attributs techniques + PPP) | — | `axia_ppp` + `axia_splynx_connector` | `axia_rbac` | — | CAP-5 (statuts contractuels) | — | `axia_splynx_connector` | `axia_billing_workflow` | — | CAP-6 (monitoring) | — | `axia_splynx_connector` | `axia_observability` | — | CAP-7 (impayés détection) | — | `axia_billing_workflow` | `axia_admin_params` | — | CAP-8 (modes suspension) | — | `axia_billing_workflow` | `axia_admin_params` | — | CAP-9 (calendaires multi-pays) | — | `axia_billing_workflow` | `axia_admin_params` | — | CAP-10 (report manuel) | — | `axia_billing_workflow` | — | — | CAP-11 (réactivation 24/7) | — | `axia_billing_workflow` | `axia_splynx_connector` | — | CAP-12 (actions techniques) | — | `axia_splynx_connector` | `axia_rbac` | — | CAP-13 (workflow impayés E2E) | — | `axia_billing_workflow` | toutes | — | CAP-14 (admin sans code) | — | `axia_admin_params` | `axia_audit` | — | CAP-15 (audit 5 ans) | — | `axia_audit` | toutes | — | (RBAC 7 rôles) | — | `axia_rbac` | toutes | — | Multi-sociétés | — | natif Odoo + record rules `axia_rbac` | toutes | — | RGPD | — | `axia_audit` (wizards) + `axia_rbac` | — |
...
| Système externe | Module Odoo proxy | Protocole | Auth | Direction | — |---|---|---|---|---| — | **Splynx API** (4 backends) | `axia_splynx_connector/services/splynx_client.py` | HTTPS REST | Token (keychain) | Sortant Odoo → Splynx | — | **Splynx Webhooks** | `axia_splynx_connector/controllers/webhook_splynx.py` | HTTPS POST | HMAC-SHA1 | Entrant Splynx → Odoo | — | **S3 (audit froid)** | `axia_audit/services/archiver_s3.py` | HTTPS S3 API | IAM key (keychain) | Sortant Odoo → S3 (WORM) | — | **Redis (queue_job)** | OCA `queue_job` natif | Redis protocol | Password (env) | Intra-cluster | — | **PostgreSQL** | Odoo ORM natif | Postgres protocol | Password (env) | Intra-cluster | — | **Prometheus** | `axia_observability/controllers/metrics.py` | HTTPS GET `/axia/metrics` | IP allow-list | Entrant Prometheus → Odoo | — | **Sentry** (optionnel) | `axia_observability/services/sentry_hook.py` | HTTPS POST | DSN (keychain) | Sortant Odoo → Sentry |
